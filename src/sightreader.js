...
function load_abc_file(filename) {
    ...
    $.ajax({
        ...
        dataType: 'text',
        success: function (data, textStatus, jqXHR) {
            original_loaded_abc = preprocess_abc_file(data);
...
function preprocess_abc_file(data) {
    const headerFieldsToIgnore = new Set(['T', 'C', 'Z', 'S', 'N', 'G', 'O', 'H', 'I', 'P', 'W', 'F', 'B']);
    let headers = [];
    let notes = [];
    ...
    for (const line of lines) {
        ...
        if (line.length >= 2 && line[1] === ':' && /^[A-Za-z]$/.test(line[0])) {
            if (headerFieldsToIgnore.has(line[0].toUpperCase())) {
